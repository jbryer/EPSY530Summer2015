...
<font color='blue'>
*A streak of length 1 means Kobe got one hit and one miss. A streak of length 0 indicates one miss and no hits.*
</font>
...
<font color='blue'>
*Kobe's mean streak length is `r mean(kobe_streak)` baskets. A streak of zero is the most typical streak length. His longest streak was 4 hits in a row.*
</font>
...
```{r}
tab_unfair <- table(sim_unfair_coin)
tab_unfair
prop.table(tab_unfair)
```

<font color='blue'>
*In this similation, we get `r tab_unfair['heads']` heads and `r tab_unfair['tails']` tails.*
</font>
...
<font color='blue'>
*We need to change the `prob` parameter to reflect a 45% success rate and change the `size` parameter to 133, or the number of observations of Kobe we have.*
</font>
...
tab_sim_streak <- table(sim_streak)
tab_sim_streak
summary(sim_streak)
barplot(tab_sim_streak)
```

<font color='blue'>
*The mean streak length is `r mean(sim_streak)`. The most common streak length is `r names(tab_sim_streak)[tab_sim_streak == max(tab_sim_streak)]`. The longest streak is `r max(sim_streak)`.*
</font>
...
tab_sim_streak2 <- table(sim_streak2)
barplot(tab_sim_streak2)
```

<font color='blue'>
*The distributions are somewhat similar. There is some differences given that these data are generated randomly.*
</font>
...
```{r}
par(mfrow=c(3,1))
barplot(table(kobe_streak), main="Kobe's Streaks", ylim=c(0,50))
barplot(tab_sim_streak, main="Simulated Streaks 1", ylim=c(0,50))
barplot(tab_sim_streak2, main="Simulated Streaks 2", ylim=c(0,50))
```

<font color='blue'>
*These three distributions are very similar. If anything, the simulated data suggests that Kobe could have had longer streaks then his longest streak of length four.*
</font>
