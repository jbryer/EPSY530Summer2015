...
```{r}
if(!file.exists('cdc.R')) {
	download.file("http://www.openintro.org/stat/data/cdc.R", "cdc.R")
}
source('cdc.R')
```
...
The command above returns the same result as the cdc[1:10,6] command. Both row-and-column no- tation and dollar-sign notation are widely used, which one you choose to use depends on your personal preference.

### A little more on subsetting

It’s often useful to extract all individuals (cases) in a data set that have specific characteristics. We accom- plish this through conditioning commands. First, consider expressions like
...
**Exercise 4** Create a new object called under23 and smoke that contains all observations of respondents under the age of 23 that have smoked 100 cigarettes in their lifetime. Write the command you used to create the new object as the answer to this exercise.
...
* If 0, the person is at their desired wieght.
* If positive, the person wants to loose weight.
* If negative the person wants to gain weight.
...
wdiff2 <- wdiff[wdiff > -200]
hist(wdiff2)
boxplot(wdiff2)
```

Most people are within 50 pounds of their desired weight. There are possible outliers that want to gain a lot of weight (i.e. more than 200 poinds).
...
There doesn't appear to be a difference, but it is difficult to tell given the possible outliers (the two largest are male).
...
mean(cdc$weight)
sd(cdc$weight)
below <- cdc$weight <= (mean(cdc$weight) - sd(cdc$weight))
above <- cdc$weight >= (mean(cdc$weight) + sd(cdc$weight))
within <- !below & !above
table(within)
prop.table(table(within))
mean(within)
```

70.76% of weights fall within on standard deviation of the mean.
