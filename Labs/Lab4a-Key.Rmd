...
<font color='blue'>
The mean area ranges from 334 to 5,642 with a mea of 1,500. The distribution is fairly normal with a few possible outliers on the positive side.
</font>
...
<font color='blue'>
The means of the two random samples are fairly close. As the sample size increases the mean should get closer to the population mean.
</font>
...
sample_means50000 <- rep(0, 50000)
for (i in 1:length(sample_means50000)) {
	samp <- sample(area, 50)
...
<font color='blue'>
There are `r length(sample_means50)` elements in `sample_mean50`. Each element represents the mean from one random sample. The mean of `sample_means50` is `r mean(sample_means50)`. It is fairly normal and is close to the population mean. As we increase the samples to 50,000 we would expect the center of the sampling distribution to get closer to the population mean.
</font>
...
**Exercise 5** To make sure you understand what you’ve done in this loop, try running a smaller version. Initialize a vector of 100 zeros called `sample_means_small`. Run a loop that takes a sample of size 50 from area and stores the sample mean in sample means small, but only iterate from 1 to 100. Print the output to your screen (type sample means small into the console and press enter). How many elements are there in this object called sample means small? What does each element represent?
...
<font color='blue'>
There are 100 elements in `sample_means_small`. Each element represents the mean of a random sample of 50 from `area`.
</font>
...
xlimits = range(sample_means10)
hist(sample_means10, breaks = 20, xlim = xlimits)
abline(v=mean(sample_means10), col='red', lwd=3)
abline(v=mean(area), col='blue', lwd=3, lty=2)
hist(sample_means50, breaks = 20, xlim = xlimits)
abline(v=mean(sample_means50), col='red', lwd=3)
abline(v=mean(area), col='blue', lwd=3, lty=2)
hist(sample_means100, breaks = 20, xlim = xlimits)
abline(v=mean(sample_means100), col='red', lwd=3)
abline(v=mean(area), col='blue', lwd=3, lty=2)
par(orig.par)
```

<font color='blue'>
As the sample size increases, the center of the distribution gets closer to the population mean (represented by the blue line).
</font>
...
<font color='blue'>
Based upon the sampling distribution, we would guess the mean price to be `r mean(sample_means50)`. The actual population mean for price is `r mean(price)`.
</font>
...
abline(v=mean(sample_means150), col='red', lwd=3)
abline(v=mean(price), col='blue', lwd=3, lty=2)
mean(sample_means150)
abs(diff(c(mean(sample_means150), mean(price))))
abs(diff(c(mean(sample_means50), mean(price))))
```

<font color='blue'>
Based upon random samples of 150, we would estimate the mean price to be `r mean(sample_means150).
</font>
...
<font color='blue'>
The `sample_means150` has a smaller spread. If making an estiamte about the population, we would want the sampling distribution with the smaller spread.
</font>
